...
#ifndef RTXDI_PT_ALLOWED_BIAS_CORRECTION
#define RTXDI_PT_ALLOWED_BIAS_CORRECTION RTXDI_BIAS_CORRECTION_RAY_TRACED
#endif
...
// Generates a pattern of offsets for looking closely around a PTven pixel.
    ...
    const RTXDI_PTTemporalResamplingParameters tparams)
{
    ...
    for (int i = 0; i < sampleCount; i++)
    {
        ...
        temporalReservoir = RTXDI_LoadPTReservoir(reservoirParams, prevReservoirPos, tparams.sourceBufferIndex);
    ...
    RTXDI_PTReservoir curReservoir = RTXDI_EmptyPTReservoir();

    float selectedTargetPdf = 0;
    if (RTXDI_IsValidPTReservoir(inputReservoir))
    {
        selectedTargetPdf = RAB_GetPTSampleTargetPdfForSurface(inputReservoir.position, inputReservoir.radiance, surface);

        RTXDI_CombinePTReservoirs(curReservoir, inputReservoir, /* random = */0.5, selectedTargetPdf);
    }
    
    if (foundTemporalReservoir)
    {
        // Found a valid temporal surface and its PT reservoir.
        ...
        if (!RAB_ValidatePTSampleWithJacobian(jacobian))
    ...
    if (foundTemporalReservoir)
    {
        ...
        float targetPdf = RAB_GetPTSampleTargetPdfForSurface(temporalReservoir.position, temporalReservoir.radiance, surface);
        
        // Combine the temporalReservoir into the curReservoir
        selectedPreviousSample = RTXDI_CombinePTReservoirs(curReservoir, temporalReservoir, RAB_GetNextRandom(rng), targetPdf);
...
#if RTXDI_PT_ALLOWED_BIAS_CORRECTION >= RTXDI_BIAS_CORRECTION_BASIC
    if (tparams.biasCorrectionMode >= RTXDI_BIAS_CORRECTION_BASIC)
    {
        ...
        if (RTXDI_IsValidPTReservoir(curReservoir) && foundTemporalReservoir)
        {
            float temporalP = RAB_GetPTSampleTargetPdfForSurface(curReservoir.position, curReservoir.radiance, temporalSurface);

#if RTXDI_PT_ALLOWED_BIAS_CORRECTION >= RTXDI_BIAS_CORRECTION_RAY_TRACED
        ...
        RTXDI_FinalizePTResampling(curReservoir, normalizationNumerator, normalizationDenominator);
    }
    else
#endif
    {
...
RTXDI_PTReservoir RTXDI_PTSpatialResampling(
    ...
    const RTXDI_RuntimeParameters params,
    const RTXDI_ReservoirBufferParameters reservoirParams,
    const RTXDI_PTSpatialResamplingParameters sparams)
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        RTXDI_PTReservoir neighborReservoir = RTXDI_LoadPTReservoir(reservoirParams, neighborReservoirPos, sparams.sourceBufferIndex);
        ...
        float targetPdf = RAB_GetPTSampleTargetPdfForSurface(neighborReservoir.position, neighborReservoir.radiance, surface);

        // The Jacobian to transform a PT sample's solid angle holds the lengths and angles to the PT sample from the surfaces,
        // that are valuable information to determine if the PT sample should be combined with the current sample's stream.
        ...
        // Valid neighbor surface and its PT reservoir. Combine the reservor.
        cachedResult |= (1u << uint(i));

        // Combine
        bool isUpdated = RTXDI_CombinePTReservoirs(curReservoir, neighborReservoir, RAB_GetNextRandom(rng), targetPdf * jacobian);
        if (isUpdated)
        {
            selected = i;
            selectedTargetPdf = targetPdf;
    ...
    if (sparams.biasCorrectionMode >= RTXDI_BIAS_CORRECTION_BASIC)
    {
        ...
        // If the PT reservoir has selected other than the initial sample, the position should be come from the previous frame.
        // However, there is no idea for the previous position of the initial PT reservoir, so it just uses the current position as its previous one.
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            // Load our neighbor's G-buffer and its PT reservoir again.
            RAB_Surface neighborSurface = RAB_GetGBufferSurface(idx, false);
            ...
            float ps = RAB_GetPTSampleTargetPdfForSurface(curReservoir.position, curReservoir.radiance, neighborSurface);
...
// A structure that groups the application-provided settings for spatio-temporal resampling.
struct RTXDI_PTSpatioTemporalResamplingParameters
{
    // Screen-space motion vector, computed as (previousPosition - currentPosition).
    // The X and Y components are measured in pixels.
    // The Z component is in linear depth units.
    ...
    // some shader cost and one approximate shadow ray per pixel (or per two pixels if checkerboard sampling is enabled).
    // Ideally, these rays should be traced through the previous frame's BVH to get fully unbiased results.
    // To enable bias correction mode, you must define RTXDI_PT_ALLOWED_BIAS_CORRECTION properly.
    uint biasCorrectionMode;
...
RTXDI_PTReservoir RTXDI_PTSpatioTemporalResampling(
    const uint2 pixelPosition,
    const RAB_Surface surface,
    RTXDI_PTReservoir inputReservoir,
    inout RAB_RandomSamplerState rng,
    const RTXDI_RuntimeParameters params,
    const RTXDI_ReservoirBufferParameters reservoirParams,
    const RTXDI_PTSpatioTemporalResamplingParameters stparams)
{
    // Backproject this pixel to last frame
    ...
    for (int i = 0; i < totalSampleCount; ++i)
    {
        ...
        const uint2 neighborReservoirPos = RTXDI_PixelPosToReservoirPos(idx, params.activeCheckerboardField);
        RTXDI_PTReservoir neighborReservoir = RTXDI_LoadPTReservoir(reservoirParams, neighborReservoirPos, stparams.sourceBufferIndex);
...
#endif // PT_RESAMPLING_FUNCTIONS_HLSLI